...
#define START_THETA_RAD (-M_PI / 2.0)   /* -pi/2: looks along -y (-90 deg) */
#endif
...
#define ROCK_APPROACH_M     0.06 /* creep until the cube reads this close, then evaluate. 2 cm
                                  * was below the ToF short-range floor so the approach never
                                  * finished -- 6 cm is reachable. */
#define COLOR_ALIGN_DIST_M    0.05   /* final-approach band: start bearing left within this (m) of the cube */
...
#define ROAM_DISTANCE_M     0.25   /* straight-line distance driven in one roam leg      */
