...
#define COL_R_WHITE  14u
#define COL_R_BLACK  16u
#define COL_G_WHITE  19u
#define COL_G_BLACK  26u
#define COL_B_WHITE  18u
#define COL_B_BLACK  23u
#define COLOR_EVAL_SAMPLES   20     /* raw colour reads averaged before classifying */
#define COLOR_WHITE_SPREAD  0.20f   /* channel-ratio spread below this = neutral -> WHITE */
...
/* color_map_to_rgb removed: color_classify now works in white-normalised ratios. */
...
{
    /* All-zero raw = pulse-measure timeout / dead OUT pin -> nothing scannable. */
    if (r_raw == 0 && g_raw == 0 && b_raw == 0) {
        if (confidence) *confidence = 0.0f;
        return SCAN_COLOR_NONE;
    }

    /* Normalise each channel by its WHITE reference. ratio < 1 = that channel is
     * BRIGHTER than white (more of that colour reflected), so the LOWEST-ratio
     * channel is the surface colour. If the three ratios are close (small spread)
     * the surface is neutral -> WHITE. Ratios (not raw counts) are the key: the raw
     * R channel reads systematically low, so absolute counts mislead but
     * white-normalised ratios do not. Verified on the debug_color 2026-06-19 means:
     * white/red/green/blue all classify correctly. (No yellow cube in this mission,
     * and the low-R bias makes green look like yellow, so yellow is not emitted.) */
    const float rr = (float)r_raw / (float)COL_R_WHITE;
    const float gr = (float)g_raw / (float)COL_G_WHITE;
    const float br = (float)b_raw / (float)COL_B_WHITE;
    ...
    if (confidence) *confidence = (spread > 1.0f) ? 1.0f : spread;

    if (spread < COLOR_WHITE_SPREAD) return SCAN_COLOR_WHITE;   /* balanced -> white  */
    if (rr <= gr && rr <= br)        return SCAN_COLOR_RED;     /* red brightest      */
    if (gr <= rr && gr <= br)        return SCAN_COLOR_GREEN;   /* green brightest    */
    return SCAN_COLOR_BLUE;                                     /* blue brightest     */
}
...
/* Pose / status / object telemetry is published to mission control over the
 * MQTT/UART0 link, rate-limited so the UART link to the ESP32 isn't flooded. */
...
#define PUB_OBJ_MAX         128  /* distinct object cells remembered for the publish */
 ...
 * treated as the same cube (covers colour-sensor jitter + odometry drift). NOTE:
 * this assumes the two robots share a comparable coordinate frame -- the same
 * assumption the existing partner-plotting code already relies on. If they start
 * at different known offsets, translate the partner's x/y before registering. */
#define MAX_CUBES           16
#define TARGET_CUBE_COUNT    6
#define CUBE_DEDUP_RADIUS_M  0.12   /* finds within 12 cm are the same cube */

static struct { double x, y; bool used; } g_cubes[MAX_CUBES];
static int g_cube_count = 0;

static bool cube_is_known(double x, double y) {
  for (int i = 0; i < MAX_CUBES; ++i) {
    if (g_cubes[i].used &&
        hypot(x - g_cubes[i].x, y - g_cubes[i].y) <= CUBE_DEDUP_RADIUS_M) {
      return true;
    }
  }
  return false;
...
/* Add a cube at (x,y) unless it is already known. Returns true ONLY when it is a
 * genuinely new cube, so the caller reports + counts it exactly once. */
static bool cube_register(double x, double y) {
  if (cube_is_known(x, y)) return false;
  for (int i = 0; i < MAX_CUBES; ++i) {
    if (!g_cubes[i].used) {
      g_cubes[i].x = x; g_cubes[i].y = y; g_cubes[i].used = true;
      ++g_cube_count;
      return true;
    }
  }
...
static color_detection_t read_color_detection(void) {
  /* Average the raw colour counts over COLOR_EVAL_SAMPLES reads, then classify the
   * mean -- a single TCS3200 read is noisy; the mean is far steadier. */
  uint64_t rs = 0, gs = 0, bs = 0;
  int navg = 0;
  for (int i = 0; i < COLOR_EVAL_SAMPLES; ++i) {
    uint32_t r = 0, g = 0, b = 0;
    color_read(&r, &g, &b);
    if (r == 0 && g == 0 && b == 0) continue;   /* skip dead / timed-out reads */
    rs += r; gs += g; bs += b; ++navg;
  }
  color_detection_t det = {0};
  if (navg == 0) { det.label = COLOR_NONE; det.detected = false; det.confidence = 0.0; return det; }
  float conf = 0.0f;
  const scan_color_t c = color_classify((uint32_t)(rs / (uint64_t)navg),
                                        (uint32_t)(gs / (uint64_t)navg),
                                        (uint32_t)(bs / (uint64_t)navg), &conf);
  switch (c) {
    ...
    /* BLACK / NONE -> background (floor/tape), not a scannable cube. */
...
static void poll_partner_mqtt(robot_state_t *s) {
  ...
  while (recv_mqtt(&px, &py, &psize, &ptemp, pcolor)) {
    if (pcolor[0] != '\0' && strcmp(pcolor, "Unknown") != 0) {
      ...
      /* Fold into the shared registry: a cube the partner located is ignored if
       * we later see it, and it counts toward the six-cube finish. Only a
       * genuinely new cube advances the count. */
 ...
 * difference in the autopilot below: this robot uses a single read (robotv2
 * averages several reads for a more reliable classification). */
static color_detection_t read_cube_color(void) {
  return read_color_detection();
}
